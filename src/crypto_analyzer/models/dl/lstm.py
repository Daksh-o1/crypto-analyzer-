"""Stacked 2-Layer LSTM Architecture."""
...
class LSTMModel(nn.Module):
    """Stacked 2-Layer LSTM with separate regression and classification heads.
    ...
    def __init__(
        ...
        # Output heads (regression scalar or classification raw logit)
        self.fc = nn.Linear(hidden_size, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, 60, K)
        lstm_out, (h_n, c_n) = self.lstm(x)
        # Use final layer's hidden state from the last sequence step
